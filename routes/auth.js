  ...
  router.post('/login', async (req, res) => {
    try {
      ...
      if (user.accountStatus === 'open') {
        ...
        return res.status(200).json({ message: 'Login successful', userId: user.userId, userData: user});
      }
...
router.post('/seller/signup', async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Error registering seller', details: err.message });
  }
});
